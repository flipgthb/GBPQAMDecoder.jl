...
aog_color_palette() = AlgebraOfGraphics.aog_theme().palette.color

# ╔═╡ 1c811aa5-1b23-4a7f-ab5d-31fc9793652a
plotly_color_palette() = map(Base.Fix1(parse,Colorant),
	split("#656efa #ef553b #00cc96 #ab63fa #ffa15a #19d3f3 #ff6692 #b6e880 #ff97ff #fecb52")
)

# ╔═╡ f3f5298d-4149-4dde-bea3-a4a0221b592d
function min_ber(df)
	subset(groupby(df,:with_noise),:ber=>(b->b.==minimum(b)))
end

# ╔═╡ 7cbc519f-4280-4aec-a73a-5a51d16fb49c
publplots(p...) = joinpath("..","plots","publ",p...)

# ╔═╡ 2b9d98e0-f1fe-4117-acdc-2fc4361eadc1
function saveplot(fig,fn; dir=publplots(),exts=["pdf","png"])
	foreach(exts) do ext
		f = joinpath(dir,join([fn,ext],"."))
		println("Trying to save file: ", f)
		save(f,fig)
	end
	println("Done.")
	return fig
...
# ╔═╡ e1a167a2-b3f5-4e81-a3d2-7646ed734984
function savedata(fn; dir=joinpath("..","data","exp_pro"))
	apply() do data
		Parquet2.writefile(
			joinpath(dir,fn*".parquet"),
			data
		)
		return data
	end
...
# ╔═╡ 601f6ed8-15f4-4d46-b0d6-8f086f528705
function generate_name(params; prefix="",suffix="",infix="__", ext="", connector="_")
	fn = @chain params begin
		pairs
		Iterators.flatten
		join(connector)
	end

	p = ifelse(isempty(prefix),"",prefix*infix)
	s = ifelse(isempty(suffix),"",infix*suffix)
	e = ifelse(isempty(ext),"","."*ext)
	return p*fn*s*e
end

# ╔═╡ ac81bbc7-c75d-449c-a6ef-f9336cdcdd86
figname(; prefix="",params...) = generate_name(params; prefix, ext="pdf") 

# ╔═╡ 3259fbd8-f47c-4a42-afb0-046f81a4c99e
md"## Data"
...
function summarize_results(
			k,T,nseq; 
			groupcols=[:power,:with_noise,:k,:with_pilot_wave],
			kw...
)
	@chain load_results_data(; k,T,nseq,kw...) begin
		DataFrames.groupby(groupcols)
		combine(
			:k=>unique⨟only,
			:signal_length=>unique⨟only=>:T,
			:ber=>mean;
			renamecols=false
		)
	end
...
function load_results_and_benchmark_data(;k,T,n,pilots,date,kw...)
	...
	@chain (rd,bd) begin
		...
		DataFrames.stack(
			[:ber_gbp,:ber_naive];
			 value_name=:ber,variable_name=:algorithm_dirty
		)
		select!(
			Not(:algorithm_dirty),
			:algorithm_dirty=>ByRow(x->Symbol(last(split(x,"_"))))=>:alg
		)
	end
end

# ╔═╡ befa9346-8aa2-4087-a554-4eda484aee5e
md"## Plotting functions"

# ╔═╡ ba61f3d9-a487-4fe0-854a-2169b907964c
# function plot_performance_simple(results_df;limits=(nothing,(10^-3.5,1)))
# 	p = data(dropmissing(results_df))*
# 		mapping(:power=>"Avg. Power [dBm]",:ber=>"BER",
# 				color=:with_noise=>"",#col=:signal_length_cat,row=:k_cat,
# 				linestyle=:alg=>" ",marker=:alg=>" ")*
# 				(visual(Scatter; markersize=12) + visual(Lines))

# 	palettes=(linestyle=[:solid,:dot],marker=[:star4,:circle])
# 	legend_info = (gbp=1,naive=2,wo_noise=1,w_noise=2)
...
# 	naive_style = [LineElement(linestyle=palettes.linestyle[legend_info.naive]),
# 				   MarkerElement(marker=palettes.marker[legend_info.naive],
# 					   			 markersize=12,
#           						 strokecolor=:black)]

# 	gbp_style = [LineElement(linestyle=palettes.linestyle[legend_info.gbp]),
# 				 MarkerElement(marker=palettes.marker[legend_info.gbp],
# 					   		   markersize=12,
#           					   strokecolor=:black)]
		
# 	wo_noise_color = MarkerElement(
# 						color=AlgebraOfGraphics.aog_theme().palette.color[legend_info.wo_noise],
# 					    marker=:rect, markersize=20
# 					)
	
# 	w_noise_color = MarkerElement(
# 						color=AlgebraOfGraphics.aog_theme().palette.color[legend_info.w_noise],
# 					    marker=:rect, markersize=20
# 					)
	

# 	l = Legend(f[1,2],
# 	    [[gbp_style,naive_style],[wo_noise_color,w_noise_color]],
# 	    [["GBP-GMM","Hard boundary"],["Without noise","With 4.5 dB noise"]],
# 		["", " "]; 
# 	    patchsize=(35, 35), rowgap=-5, groupgap=-5
# 	)
...
function plot_performance_publication(results_df;
	...
	title = let
		T = skipmissing(results_df.T)|>unique|>only
		k = skipmissing(results_df.k)|>unique|>only
		"Signal with $(T) symbols,   Mixture with $(k) components"
	end
	...
	p = data(dropmissing(results_df))*
		mapping(:power=>"Avg. Power [dBm]",:ber=>"BER",
				...
				linestyle=tag,
				marker=tag)* 
				(visual(Scatter; scatter_kw...) + visual(Lines; lines_kw...))

	kw = (; 
		draw_kw..., 
		axis=merge((;title,limits=(nothing,(10^-5,1)),yscale=log10),get(draw_kw,:axis,(;)))
	)
	draw(p; kw...,)
end

# ╔═╡ 19c491d0-b361-4369-a143-5bd752aa07f2
function plot_performance_ratio_publication(results_df; 
			scatter_kw=(;markersize=15), lines_kw=(;linewidth=3),
			draw_kw...
		)
	...
	df = @chain results_df begin
			unstack(:alg,:ber)
			transform!(
				[:gbp,:naive]=>ByRow((x,y)->y/(x+eps()))=>:ber_naive_vs_ber_gbp
			)
			select!(Not([:gbp,:naive]))
		end
	
	title = let
		T = skipmissing(df.T)|>unique|>only
		k = skipmissing(df.k)|>unique|>only
		"Signal with $(T) symbols,   Mixture with $(k) components"
	end
	
	p = data(dropmissing(df))*
		mapping(:power=>"Avg. Power [dBm]",:ber_naive_vs_ber_gbp=>"BER Hard boundary vs BER GBP",
				color=tag,
				linestyle=tag,
				marker=tag)* 
				(visual(Scatter; scatter_kw...) + visual(Lines; lines_kw...))
	...
	kw = (; 
		draw_kw..., 
		axis=merge((;title),get(draw_kw,:axis,(;)))
	)
	draw(p; kw...,)
end
...
function plot_benchmark(pilots; 
	...
	dc = AlgebraOfGraphics.aog_theme().palette.color
	
	wo_noise_color = make_legend_element(;
		linestyle,
		color=get(draw_kw,:palettes,(;color=dc)).color[1],
		marker,markersize
	)
	
	w_noise_color = make_legend_element(;
		linestyle,
		color=get(draw_kw,:palettes,(;color=dc)).color[2],
		marker,markersize
	)
	

	l = Legend(f[1,2],
	    [wo_noise_color,w_noise_color],
	    ["Without noise","With 4.5 dB noise"]; 
...
function plot_opt_ber_signal_lenght(;k,chunks,chunk_parts,chunk_step,batch=1,resdir="")

	boptdf = subset(
		subset(
			groupby(load_benchmark_data(),[:with_noise]),
			:ber=>(b->b.==minimum(b));
			ungroup=false
		),
		:power_dbm=>(p->p.==maximum(p))
	)

	select!(boptdf,Not(:alg),:alg=>ByRow(a->"$a")=>:alg)

	optdf = mapreduce(vcat,chunks,chunk_parts,chunk_step) do chunk,parts,step
		load_results_data_table(
			;k,chunk,parts,step,batch,resdir
		)|>min_ber
	end

	select!(optdf,Not([:signal_length,:number_of_gaussians]),
		[:signal_length,:number_of_gaussians,:alg]=>ByRow((T,K,a)->"$a T=$T K=$K")=>:alg
	)
	...
	n = :with_noise=>renamer([true=>"With 4.5dB noise", false=>"Without noise"])=>" "

	data(df)*
		mapping(
			:alg_pos=>"",
			:ber=>(b->b+eps())=>"Optimal BER",
			dodge=n,color=n)*
		visual(Scatter; marker=:circle,markersize=15)|>
		# visual(BarPlot)|>
		p->draw(p;
			axis=(;
				limits=(nothing,(10^-6,10^-1)),
				aspect=3,xticks=(0:3,["Hard\ndecision", "T=5", "T=10", "T=50"]),
				title="Mixture with $(k) components"
			)
		)
end

# ╔═╡ 045e667b-791c-47ed-a4ff-51f0f241ea5f
function plot_opt_ber_k(;ks,chunk,parts,step,batch=1,resdir="")
	...
	data(df)*
		...
		p->draw(p; 
			axis=(;
				...
				title="Signal with $(chunk) symbols"
			)
		)
end
		...
		@chain (;k,T,n,pilots,date) begin
			...
			plot_performance_publication(_;
				...
				palettes=(;
					...
					color=plotly_color_palette()[[1,3,2,4]]
				), 
				legend=(;
						rowgap=12,
...
git-tree-sha1 = "8c33d7385f2c9f55d2b47bf9005d40b8bddb2758"
